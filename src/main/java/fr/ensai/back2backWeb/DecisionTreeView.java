...
public class DecisionTreeView extends Composite implements View {
	...
	// ntree params
	Slider minPerLeaf = new Slider("Min. number of instances in each leaf", 1, 100);
	Slider minSplit = new Slider("Min. number of instances to split a node", 1, 100);
	...
	Slider maxDepth = new Slider("Maximum depth", 1, 30);
...
//		gini.setSelectedItem("True");
	...
	//	dtForm.addComponents(propTrain, maxDepth, minSplit, minPerLeaf, maxBins, cp, gini, nbIter);
		dtForm.addComponents(propTrain, maxDepth, minSplit, minPerLeaf, maxBins, cp, nbIter);
		...
		submit.addClickListener(e -> {
			...
			dtree.setGini(((MyUI) getUI()).data.isClassif());
			...
			// String tmp = ((MyUI) getUI()).data.isClassif() ?
	...
	public Panel getComparisonPanel() throws Exception {
		...
		WekaDecisionTree wdt = new WekaDecisionTree(((MyUI) getUI()).data, ((MyUI) getUI()).dtree,
				propTrain.getValue());
		RDecisionTree rdt = new RDecisionTree(((MyUI) getUI()).data, ((MyUI) getUI()).dtree, propTrain.getValue());
		SparkDecisionTree sdt = new SparkDecisionTree(((MyUI) getUI()).data, ((MyUI) getUI()).dtree,
				propTrain.getValue());
